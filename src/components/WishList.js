function WishList({setWatchList,watchList}){
    const number=watchList.length;
    const totalDuration = watchList.reduce((acc,watch)=>acc+Number(watch.Runtime.split(" ")[0]),0)
    function handleDeleteWatch(id){
        setWatchList((watchList)=>watchList.filter((watch)=>watch.imdbID !== id))
    }
    return(
    <>
        <div className="rating" style={{width:'100%'}}>
            ...
            <div className="list" key={watch.imdbID}>
                <div className="btn" onClick={()=>handleDeleteWatch(watch.imdbID)} style={{position:'absolute',right:'10px'}}>
                ...
                <div className="movie-details">
                    ...
                    <div className="flex">
                        ...
                        {watch.myRating > 0 && <div className="second"><i style={{color:'orange', marginRight:'5px'}} className="fa-solid fa-star"></i>{watch.myRating}</div>}
                        <div className="second"><i class="fa-solid fa-hourglass-end" style={{color:'rgb(115, 107, 107)' ,marginRight:'5px'}}></i>{watch.Runtime}</div>
                    </div>
                </div>
            </div>
            ))}
